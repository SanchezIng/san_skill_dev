...
### Addendum — la primera versión del hook no funcionaba (instancia del hallazgo 6)

El hook original leía la ruta del archivo de una variable de entorno
`CLAUDE_TOOL_FILE_PATH` **que no existe**, y escribía el aviso por stdout plano.
El contrato real de `PreToolUse` es otro:

1. La ruta llega en el **JSON de stdin** (`.tool_input.file_path`, o
   `.tool_input.notebook_path` en `NotebookEdit`).
2. El stdout plano de un `PreToolUse` va **solo al log de depuración**: no lo ve
   ni Claude ni el usuario. Para que el aviso llegue al modelo hace falta
   `hookSpecificOutput.additionalContext`. (Los eventos que sí inyectan stdout
   como contexto son `UserPromptSubmit`, `UserPromptExpansion` y `SessionStart`
   — por eso `arranque.sh` sí funcionaba y este no.)

Resultado: el hook hacía `exit 0` en la primera línea **siempre**. Un mecanismo
que se ejecuta en cada edición y no hace nada, dando por cerrado el hallazgo 1.

Lo revelador es **por qué pasó la verificación**: se probó contra el contrato
imaginado (poniendo a mano la variable inventada), no contra el real. Es el
hallazgo 6 exacto — *el paso hizo su trabajo y el resultado era falso, porque lo
comprobado no era lo que importaba*.

Cómo se detectó y cómo está verificado ahora:

| Comprobación | Antes | Ahora |
|---|---|---|
| Suite con el payload real (9 casos) | 4 fallos | 9/9 OK |
| Edición real vía `claude -p`, contando avisos en el transcript | **0** | **2** (uno como `hook_additional_context`) |
| ¿Los tests muerden? (contra la versión rota) | — | sí: 4 fallos |

La suite (`plantillas/hooks/test_recordar-seguridad.sh`) viaja con el hook y se
instala en el proyecto: **un hook silencioso es indistinguible de un hook roto**,
así que tiene que poder comprobarse sin leer el código.

**Límite conocido:** el hook cubre `Edit|Write|NotebookEdit`. Un merge resuelto
solo con comandos git (`git checkout --theirs`) mete código sin pasar por
ninguna de esas herramientas. Para ese caso la red es el checkpoint de
`/cerrar-sesion`, no el hook.
